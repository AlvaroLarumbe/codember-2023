...
fn main() {
    ...
    let mut order_tracker: Vec<&str> = Vec::new();
    let mut usages_tracker: HashMap<&str, u64> = HashMap::new();
    
    for word in text.split_whitespace() {
        if !usages_tracker.contains_key(word) {
            order_tracker.push(word);
        }

        let count = usages_tracker.entry(word).or_insert(0);
        *count += 1;
    }

    for word in order_tracker {
        print!("{0}{1}", word, usages_tracker.get(word).unwrap());
    }
